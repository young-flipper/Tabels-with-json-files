  ...
  data.forEach(task => { // Перебираем каждый объект задачи из данных
    const row = `
      <tr>
        ...
        <td>${task.numberSP}</td> <!-- Отображаем номер СП -->
        <td>${task.designSP}</td> <!-- Отображаем дизайн СП -->
        <td>${task.nameSP}</td> <!-- Отображаем название СП -->
        <td>${task.typeSizeSP}</td> <!-- Отображаем тип/размер СП -->
        <td>${task.numberOperation}</td> <!-- Отображаем номер операции -->
        <td>${task.nameOperation}</td> <!-- Отображаем название операции -->
        <td>${task.countIssued}</td> <!-- Отображаем количество выданых единиц -->
        <td>${task.countAccepted}</td> <!-- Отображаем количество принятых единиц -->
        <td>${task.percentage}%</td> <!-- Отображаем процент выполнения -->
      </tr>
    `;
